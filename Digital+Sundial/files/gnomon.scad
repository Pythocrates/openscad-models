...
module build_block(gnomon_thickness, char_list, char_angles, pixel_wall_angles) {
    /* Build a block with a set of characters */
    difference() {
        // Build the gnomon shape
        intersection() {
            cuboid([gnomon_thickness, 2 * gnomon_radius, gnomon_radius], align=V_TOP);
            xcyl(r=gnomon_radius, h=gnomon_thickness, center=true, $fn=FN);
        }
...
module build_round_top_block() {
    ...
    color("green") {
        intersection() {
            cuboid([gnomon_thickness, 2 * gnomon_radius, gnomon_radius], align=V_TOP);
            xmove(gnomon_radius / 2) xscale(0.3) sphere(r=gnomon_radius, $fn=FN);
        }
    }
}
...
module Block_hours_tens() {
    color("yellow") single_character_block(characters="1111111", pixel_wall_angle_y=6.0);
}


module Block_hours_units() {
    single_character_block(characters="0123456", pixel_wall_angle_y=6.0);
}


module Block_minutes_tens() {
    color("blue") single_character_block(characters="0240240240240240240", pixel_wall_angle_y=1.0);
    //char_angle_y = [-50,-45,-40, -35,-30,-25, -20,-15,-10, -5,0,5, 10,15,20, 25,30,35, 40];
}


module Block_minutes_units() {
    color("red") single_character_block(characters="0000000", pixel_wall_angle_y=8.0);
}


module Block_semicolon() {
    color("blue") single_character_block(characters=":::::::", pixel_wall_angle_y=8.0);
}


module single_character_block(characters, pixel_wall_angle_y) {
