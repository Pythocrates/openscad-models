...
module rounded_corner(xdir, ydir) {
    translate(35.5 * [xdir, ydir, 0]) cylinder(r=11, h=3, center=true);
}
...
module foot(xdir, ydir) {
    translate([0, 0, -1.5]) translate(35.5 * [xdir, ydir, 0]) cylinder(r=10, h=3, center=true);
}
...
difference() {
    union() {
        ...
        cube([73, 93, 3], center=true);
        cube([93, 73, 3], center=true);
    }
    union() {
        styled_text("Wernis", -40, 4);
        styled_text("Weizen", -30, -18);
        for (xdir = [-1:1]) for (ydir = [-1, 1]) hop(xdir, ydir);
        for (xdir = [-1, 1]) for (ydir = [-1, 1]) foot(xdir, ydir);
    }
}
